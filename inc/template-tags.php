...
if ( ! function_exists( 'avani_posted_on' ) ) :
	...
	function avani_posted_on() {
		...
		$posted_on = sprintf(
			_x( '发布与 %s', 'post date', 'avani' ),
			'<a href="' . esc_url( get_permalink() ) . '" rel="bookmark">' . $time_string . '</a>'
		);

		$byline = sprintf(
			_x( '该文章 由 %s', 'post author', 'avani' ),
	...
	function avani_entry_footer() {
		...
		if ( is_singular( 'post' ) ) {
			...
			if ( $categories_list && avani_categorized_blog() ) {
				printf( '<span class="cat-links">' . __( '分类 %1$s', 'avani' ) . '</span>', $categories_list );
			}

			/* translators: used between list items, there is a space after the comma */
			$tags_list = get_the_tag_list( '', __( ', ', 'avani' ) );
			if ( $tags_list ) {
				printf( '<span class="tags-links">' . __( '标签 %1$s', 'avani' ) . '</span>', $tags_list );
			}
		}
	}
endif;
